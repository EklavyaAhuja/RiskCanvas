...
const CardSwap = forwardRef(function CardSwap({
  ...
  const pendingOrderRef = useRef(null); // stores the order after current swap completes

  useImperativeHandle(externalRef, () => ({
    next: () => {
      // Commit pending order if mid-animation
      if (pendingOrderRef.current) {
        order.current = pendingOrderRef.current;
      ...
      // Reset physical positions to match order.current immediately
      ...
      // Clear interval and trigger fresh swap
  ...
  useEffect(() => {
    ...
    const swap = () => {
      ...
      // Store what order will be after this swap completes
      pendingOrderRef.current = [...rest, front];
      tl.call(() => {
