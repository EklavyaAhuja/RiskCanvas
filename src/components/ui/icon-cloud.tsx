...
export function IconCloud({ icons, images }: IconCloudProps) {
  ...
  useEffect(() => {
    ...
    // ↑ use actual DPR so retina screens get crisp icons

    const ICON_SIZE = 80          // logical px
    const CANVAS_SIZE = ICON_SIZE * DPR  // physical px
    ...
    const newIconCanvases = items.map((item, index) => {
      ...
      if (offCtx) {
        // Scale all drawing by DPR so 1 logical px = DPR physical px
  ...
  useEffect(() => {
    ...
    const dpr = window.devicePixelRatio || 2;
    if (canvas.width !== 400 * dpr) {
      canvas.width = 400 * dpr;
      canvas.height = 400 * dpr;
      ctx.scale(dpr, dpr);
    }

    const ICON_SIZE = 80  // must match offscreen logical size above

    const animate = () => {
      ctx.clearRect(0, 0, 400, 400) // Clear the logical space
      ...
      const speed = 0.0005 + (distance / maxDistance) * 0.002
      ...
      iconPositions.forEach((icon, index) => {
        ...
        ctx.save()
        ctx.translate(200 + rotatedX, 200 + rotatedY)
        ctx.scale(scale, scale)
        ctx.globalAlpha = opacity

        if (icons || images) {
          if (iconCanvasesRef.current[index] && imagesLoadedRef.current[index]) {
            // Draw using logical size — canvas internally stores at DPR resolution
            ctx.drawImage(
  ...
  return (
    <canvas
      ...
      className="w-[150%] max-w-none h-auto object-contain cursor-grab active:cursor-grabbing"
