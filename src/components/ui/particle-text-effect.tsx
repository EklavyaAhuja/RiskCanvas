...
class Particle {
  ...
  move() {
    ...
    // Add physical friction/damping to prevent infinite micro-jittering around the target
    this.vel.x *= 0.95
    this.vel.y *= 0.95
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    offscreenCtx.font = "bold 100px 'Inter'"
  ...
  const animate = () => {
    ...
    // Check if particles have mathematically settled into place
    let allSettled = true;
    ...
    for (let i = 0; i < particles.length; i++) {
       ...
       if (!p.isKilled) {
         ...
         if (dist > 5.0) { // Still moving into position
            allSettled = false;
            break;
         }
       }
    ...
    if (allSettled && activeParticles > 0) {
      frameCountRef.current++;
      // Wait for roughly 1.5 seconds (90 frames) holding the perfectly formed text before transitioning
      ...
      frameCountRef.current = 0; // Reset timer if physics is still resolving
    }
